...
class DepthwiseSeparableConvBlock(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.cbamm = BAMM(out_channels, pool_types=pool_types)
    def forward(self, x):
...
class LinearBottleneck(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if kernel_size == 3:
            ...
            raise ValueError
        if self.use_se:
            self.cbamm = BAM(out_channels, pool_types=pool_types)
...
class MobileNetV3(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if final_conv_se:
            self.backbone.add_module("final_se", BAM(in_channels, pool_types=pool_types))
