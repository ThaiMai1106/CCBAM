...
class SpatialGate_new1(nn.Module):
    def __init__(self, gate_channel, reduction_ratio=16):
        ...
        self.gate_s = nn.Sequential(
            nn.Conv2d(gate_channel, cout, kernel_size=1),
            nn.BatchNorm2d(cout),
            nn.ReLU(),

            nn.Conv2d(cout, cout, kernel_size=3, stride=1, padding=1, groups=cout, bias=False),
            nn.BatchNorm2d(cout),
            nn.ReLU(),

            nn.Conv2d(cout, 1, kernel_size=1)
        )

    def forward(self, in_tensor):
        x_gate_s = self.gate_s(in_tensor)  # [B,1,H,W]
        return x_gate_s
...
class SpatialGate_LKA_Lite(nn.Module):
    def __init__(self, gate_channel, reduction_ratio=16):
        ...
        self.gate_s = nn.Sequential(
            ...
            nn.Conv2d(mid, mid, kernel_size=7, padding=3, dilation=1,
